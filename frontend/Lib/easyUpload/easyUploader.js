	...
	easyUploader.prototype = {
		...
		buildUploader: function() {
			...
			html += '<div class="easy-uploader">' +
				...
				// +  '</span>'
				+
				'<i style="display: none !important;" class="btn-check-all cursor-select checkbox unchecked"></i>' +
			...
			var buildCheckBox = function(fileItem) {
				...
				matchProgressText = function(fileItem) {
					if(fileItem.uploadPercentage == 0) {
						if(fileItem.ajaxResponse && fileItem.ajaxResponse.code != 'undefined' && fileItem.ajaxResponse.code == 200) {
							value = fileItem.ajaxResponse.data[0].FUJIAN_ID;
							alert(value);
						}
		...
		bindListEvent: function() {
			...
			$(".btn-delete-item").off("click").on("click", function() {
				...
				if((that.files[that.ajax.index] && that.files[that.ajax.index].id == id) && that.files[that.ajax.index].uploadStatus == 'loading') {
					...
					that.updateFilesDom();
					//that.configs.onRemove && that.configs.onRemove(message);
					that.configs.onRemove && that.configs.onRemove(new_files, arr);
					that.ajax.isReady = true;
				}
			});
			...
			timer = setInterval(function() {
				if(that.fileObj.isReady) {
					...
					if(that.files.length < that.configs.maxCount && index < that.fileObj.fileList.length) {
						...
						countDiff = that.fileObj.fileList.length + existingCout - that.configs.maxCount,
				...
				if(hasChecked) {
					if(checkedWaitingFiles.length) {
						...
						this.ajax.example = $.ajax({
							...
							// },
							// beforeSend: function () {
							//     that.files[that.ajax.index].uploadPercentage = 10;
							//     that.updateFilesDom();
							// },
							success: function(res) {
								//                          	console.log(res)
								if(res[that.configs.successKey] == that.configs.successValue) {
									var uploadPro = setInterval(function() {
										if(that.files[that.ajax.index].uploadPercentage != 100) {
											that.files[that.ajax.index].uploadPercentage += 10;
											that.updateFilesDom();
										}
									}, 100);
									setTimeout(function() {
										clearInterval(uploadPro);
