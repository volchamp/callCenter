...
;(function(window, document) {
    ...
    easyUploader.prototype = {
        ...
        buildUploader: function() {
            ...
            + '</div><i id="tips"></i><i id="tips_1"></i>'
            +  '<ul id="upload-list" class="list">'
            +  '</ul>'
            +  '</div>';
            return html;
        },
        ...
        bindHeadEvent: function() {
            ...
            $(".btn-upload-file").off("click").on("click", function () {
                ...
                var tipFiles = that.files;
                if (tipFiles == "") { layer.msg('请选择您需要上传的附件!');}
            ...
            $(".btn-delete-file").off("click").on("click", function () {
                ...
                var tipFiles = that.files;
                if (tipFiles == "") { layer.msg('暂没有可删除的附件!'); }
                that.files.forEach(function (item) {
                ...
                if (hasLoading) {
                    ...
                    that.files = newFiles;
                    that.updateFilesDom();
                    that.configs.onRemove && that.configs.onRemove(newFiles);
                    that.ajax.isReady = true;
                }
            });
