...
(function($, undefined) {
    ...
    var LANG = {
        zh_cn: {
            'errorCannotGetDataFromRemote': '无法从远程服务器获取数据。',
        ...
        if (dataSource.array) {
            ...
            if (loadData) {
                ...
                loadData(params, function(resultData, error) {
                    ...
                    //start add by zb 无数据时提示用户并重新渲染表格
                    if(resultData.pager.recTotal==0)
                    {
                        that.showMessage(resultData.message, 'info');
                        that.setDataSource([]);
                        that.render();
                        return;
                    }
    ...
    DataGrid.prototype.showMessage = function(message, type, autoCloseTime) {
        if(type=="danger")
        {
            $.zui.messager.show(message, {placement: 'center',type:'danger',time:2500,icon:'warning-sign'});
        }
        else
        {
            $.zui.messager.show(message, {placement: 'center',type:'info',time:2500,icon:'info-sign'});
        }
